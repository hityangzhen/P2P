...
int main(int argc, char *argv[])
{
    ...
    signal(SIGHUP, (void (*)(int))shutdown_handler);//关闭终端
    signal(SIGINT, (void (*)(int))shutdown_handler);//按下CTRL+C
    //系统默认的终止信号
    signal(SIGTERM, (void (*)(int))shutdown_handler);//kill (1)命令
    ...
    //此标志置位表示调用者将在bind函数调用中使用返回的地址结构，如果不设置就表示将在connect函数中使用
...
void *tcp_listen()
{
    ...
    while(1)
    {
        ...
        else
        {
            ...
            thpool.AddWork(new Job(p2p, (void *)&params));//添加到线程池
        }
    }
}